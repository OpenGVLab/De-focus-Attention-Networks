...
def build_optimizer(config, model, logger, simmim=False, is_pretrain=False):
    """
    ...
    if simmim:
        ...
        parameters = set_weight_decay(model, logger, skip, skip_keywords, config)
...
#                 offset_no_decay.append(param)
#             else:
#                 offset_decay.append(param)
...
def set_weight_decay(model, logger, skip_list=(), skip_keywords=(), config=None):
    has_decay = []
    no_decay = []
    offset_decay = []
    offset_no_decay = []
    ...
    for name, param in model.named_parameters():
        ...
        if 'offset' in name:
        ...
        elif 'freq' in name and config.MODEL.theta_no_decay:
            if config.MODEL.theta_lr_scale == 1.0:
                no_decay.append(param)
                logger.info(f"{name} has no weight decay")
            else:
                offset_no_decay.append(param)
            ...
            no_decay.append(param)
            logger.info(f"{name} has no weight decay")
        elif any([k in name for k in ['A', 'A_log', 'D']]):
            no_decay.append(param)
            logger.info(f"{name} has no weight decay")
        else:
            has_decay.append(param)
    return [{'params': has_decay},
            {'params': no_decay, 'weight_decay': 0.},
            {'params': offset_decay, 'lr_scale': 0.1},
            {'params': offset_no_decay, 'lr_scale': 0.1, 'weight_decay': 0.},
            ]
